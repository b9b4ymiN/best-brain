...
describe('worker fabric', () => {
  ...
  test('does not retry claude provider-unavailable before falling back to codex', async () => {
    const claudeSequence = new SequenceWorkerAdapter('claude', [
      {
        summary: 'Claude provider is unavailable because authentication is missing on this machine.',
        status: 'failed',
        failure_kind: 'provider_unavailable',
    ...
    expect(claudeSequence.calls).toBe(1);
    expect(dispatch.executed_worker).toBe('codex');
    expect(dispatch.manager_result.attempted_workers).toEqual(['claude', 'codex']);
  });
